...
public class TioPatinhasMIDlet
    ...
    public TioPatinhasMIDlet() {
        ...
        if (supports != null && supports.length() > 0) {
            ...
            mMainForm.addCommand(mCaptureCommand);
        } else {
    ...
    public void capture() throws IOException {
        ...
        int[] ARGB = PegaBytesImagem();
        //ImageProcessor.initializeImage(image);
        //ImageProcessor.process(image);
        Image img = Image.createRGBImage(ARGB, Larg, Alt, false);

        TParamsRC ParamsRC = new TParamsRC("",
                new CTonsCinza(img), new CBitmap(img));
    ...
    private int[] PegaBytesImagem() throws IOException {
        int x, y;
        int LARG = 320, ALT = 240;
        ...
        for (y = ALT - 1; y >= 0; y--) {
            for (x = 0; x < LARG; x++) {
                indice_vet=y*LARG+x;
                retorno[indice_vet] = temp[indice++];
                retorno[indice_vet] += temp[indice++] << 8;
                retorno[indice_vet] += temp[indice++] << 16;
            }
        }
        return retorno;
    }
